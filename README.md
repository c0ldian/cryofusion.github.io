...
- 继保调试仪步骤化参数：每一步包含通道电流幅值/相角、频率、加量保持时间、超时判据
...
- 调试仪六通道输出建议（UA/UB/UC/IA/IB/IC）以及预置故障保持时间
...
### 2026-02-27 — 差动/距离算法重构与调试参数输出

- 修复差动补偿矩阵应用公式中的相量乘法错误，确保三相补偿计算正确
- 距离保护输出测量阻抗复数（R/X）与阻抗角，方向判据使用真实复阻抗向量
- 差动与距离模块均新增继保调试仪参数结构化输出（电压/电流/相角/时间）
